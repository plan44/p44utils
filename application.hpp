...
namespace p44 {
  ...
  public:
    ...
    /// @param aFreePath must be set to allow free path specifications, i.e. subdirectories and absolute paths.
    ...
    PathType getPathType(const string aPath, bool aFreePath, bool aTempPrefixOnly, size_t* aPrefixLenP = nullptr);
    ...
    /// @param aTempPrefixOnly if set, only temp prefix is allowed
    /// @return the path type or notallowed when not allowed
    PathType extractPathType(string& aPath, bool aFreePath, bool aTempPrefixOnly);

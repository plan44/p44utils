...
static ssize_t _modbus_rtu_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
    ...
    if (ctx_rtu->rts != MODBUS_RTU_RTS_NONE) {
        ...
        if (ctx->debug) {
            fprintf(stderr, "Sending message using RTS signal\n");
        }
...
static void _modbus_rtu_receive_finish(modbus_rcv_t* rcvctx)
{
    ...
    if (ctx_rtu->confirmation_to_ignore) {
        ...
        if (rcvctx->msg_length==0) {
            /* It wasn't a message for this slave, next expected message on the bus is a confirmation from another slave to ignore */
...
static int _modbus_rtu_connect(modbus_t *ctx)
{
    ...
    if (ctx_rtu->baud==1) {
        if (ctx->debug) {
            printf("Opening %s without setting serial params\n", ctx_rtu->device);
        }
    }
    else {
        if (ctx->debug) {
        ...
        if (mode == MODBUS_RTU_RS485) {
            ...
            if (ctx_rtu->serial_mode == MODBUS_RTU_RS485) {
                ...
                if (ioctl(ctx->s, TIOCSRS485, &rs485conf) < 0) {
                    ...
                    if (ctx->debug) {
                        fprintf(stderr, "TIOCSRS485 failed (%d), but tolerated because custom RTS handler is set (set_rts/set_rts_ex)\n", errno);
                    }
                }
            }
...
static void _forward_set_rts(modbus_t *ctx, int on, void *cbctx)
{
  void (*f)(modbus_t *ctx, int on) = (void (*)(modbus_t *ctx, int on))(cbctx);
  f(ctx, on);
}

int modbus_rtu_set_custom_rts(modbus_t *ctx, void (*set_rts) (modbus_t *ctx, int on))
{
  return modbus_rtu_set_custom_rts_ex(ctx, _forward_set_rts, set_rts);
}


int modbus_rtu_set_custom_rts_ex(modbus_t *ctx, void (*set_rts_ex) (modbus_t *ctx, int on, void *cbctx), void *cbctx)
{
    if (ctx == NULL) {
        errno = EINVAL;
        return -1;
    ...
    if (ctx->backend->backend_type == _MODBUS_BACKEND_TYPE_RTU) {
        modbus_rtu_t *ctx_rtu = ctx->backend_data;
        ctx_rtu->set_rts_ex = set_rts_ex;
        ctx_rtu->set_rts_cbctx = cbctx;
        return 0;
    } else {
        errno = EINVAL;
        return -1;

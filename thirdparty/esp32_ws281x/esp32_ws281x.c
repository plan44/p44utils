...
void ws281x_copy()
{
  ...
  offset = ws281x_half * MAX_PULSES;
  ws281x_half = !ws281x_half;

  len = ws281x_len - ws281x_pos;
  if (len > (MAX_PULSES / 8))
    len = (MAX_PULSES / 8);
  ...
  for (i = 0; i < len; i++) {
    bit = ws281x_buffer[i + ws281x_pos];
    for (j = 0; j < 8; j++, bit <<= 1) {
      RMTMEM.chan[RMTCHANNEL].data32[j + i * 8 + offset].val =
        ws281x_bits[(bit >> 7) & 0x01].val;
    }
...
void ws281x_handleInterrupt(void *arg)
{
  ...
  else if (RMT.int_st.ch0_tx_end && ws281x_sem) {
...
void ws281x_init(int gpioNum)
{
  ...
  ws281x_bits[0].duration0 = PULSE_T0H;
  ws281x_bits[0].duration1 = PULSE_T0L;
  ws281x_bits[1].level0 = 1;
  ws281x_bits[1].level1 = 0;
  ws281x_bits[1].duration0 = PULSE_T1H;
  ws281x_bits[1].duration1 = PULSE_T1L;
...
void ws281x_setColors(unsigned int length, rgbVal *array)
{
  ...
  ws281x_sem = xSemaphoreCreateBinary();

  RMT.conf_ch[RMTCHANNEL].conf1.mem_rd_rst = 1;
  RMT.conf_ch[RMTCHANNEL].conf1.tx_start = 1;

  xSemaphoreTake(ws281x_sem, portMAX_DELAY);
  vSemaphoreDelete(ws281x_sem);
  ws281x_sem = NULL;
